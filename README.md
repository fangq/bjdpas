...
`bjdata.pas` is the whole library: one unit, one class, no dependencies
beyond `Classes`, `SysUtils` and `Math`. Drop it into a project and add
`bjdata` to the `uses` clause, or open `bjdatapkg.lpk` to install it into
Lazarus as a package.
...
begin
  ...
  try
    ...
    grid := TBJData.NewNDArray('D', [128, 128, 64]);      // 3-d double array
    grid.SetElem(grid.Offset([0, 0, 0]), 3.5);
...
When it pays off, measured on the files used for the benchmarks below:
